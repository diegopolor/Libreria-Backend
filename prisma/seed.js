...
async function main() {
  ...
  // 1. Limpieza de datos previa (orden de dependencias inverso)
  ...
  const librarian = await prisma.user.create({
    data: {
      ...
      passwordHash: passwordHashLibrarian,
      name: 'María la Bibliotecaria',
      role: 'LIBRARIAN',
    },
  });
  ...
  const softwareCat = await prisma.category.create({
    data: {
      ...
      description: 'Libros sobre lenguajes de programación, patrones de diseño y desarrollo web.',
    },
  });

  const mathCat = await prisma.category.create({
    data: {
      name: 'Matemáticas y Ciencia',
      description: 'Textos de cálculo, física, álgebra lineal y ciencias naturales.',
    },
  });

  const fictionCat = await prisma.category.create({
    data: {
      name: 'Ciencia Ficción',
      description: 'Literatura fantástica, distopías y viajes espaciales.',
    },
  });
  ...
  const booksData = [
    ...
    },
    {
      ...
      isbn: '978-0441172719',
      totalCopies: 1, // Excelente para probar colas de espera
      availableCopies: 1,
