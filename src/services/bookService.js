...
<<<<<<< HEAD
    if (existing) {
      throw new Error('Ya existe un libro registrado con este ISBN.');
    }

    // Asegurarse de que al crearse, las copias disponibles sean iguales a las copias totales
...
<<<<<<< HEAD
    ...
    if (edition) {
      where.edition = { contains: edition, mode: 'insensitive' };
    }
    if (year) {
    ...
    // Ordenamiento
    const orderBy = {};
    if (sortBy === 'alphabetical' || sortBy === 'title') {
      orderBy.title = sortOrder === 'desc' ? 'desc' : 'asc';
    } else if (sortBy === 'publicationDate' || sortBy === 'date') {
      orderBy.publicationDate = sortOrder === 'desc' ? 'desc' : 'asc';
    } else {
      orderBy.createdAt = 'desc'; // Por defecto, orden inverso de creación
    }
      ...
      include: {
        ...
        _count: {
          select: {
            ...
            loanRequests: { where: { status: 'PENDING' } },
          },
        },
      },
    });
...
<<<<<<< HEAD
    ...
    // Calcular nuevas copias disponibles si cambia el total de copias
    ...
    if (data.totalCopies !== undefined) {
      ...
      if (availableCopies < 0) {
        throw new Error('No puedes reducir el total de copias por debajo del número de copias actualmente prestadas.');
      }
    }
  ...
  static async deleteBook(id) {
    ...
    if (existing.loans.length > 0) {
      throw new Error('No se puede eliminar el libro porque hay préstamos activos pendientes de devolución.');
    }
...
=======
    ...
    if (data.totalCopies !== undefined) {
      ...
      if (availableCopies < 0) throw new Error('No puedes reducir el total de copias por debajo del número de copias actualmente prestadas.');
    }
    const updateData = { ...data };
